...
.PHONY: build
build:
	CGO_ENABLED=0 go build ${LDFLAGS} -v -o target/go-infoblox-exporter .
...
build-release-amd64:
	CGO_ENABLED=0 GOOS=linux   GOARCH=amd64 go build ${LDFLAGS} -o=go-infoblox-exporter.linux.amd64 .        && \
	CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build ${LDFLAGS} -o=go-infoblox-exporter.windows.amd64.exe .  && \
	CGO_ENABLED=0 GOOS=darwin  GOARCH=amd64 go build ${LDFLAGS} -o=go-infoblox-exporter.darwin.amd64 .

.PHONY: build-release-arm64
build-release-arm64:
	CGO_ENABLED=0 GOOS=linux   GOARCH=arm64 go build ${LDFLAGS} -o=go-infoblox-exporter.linux.arm64 .        && \
 	CGO_ENABLED=0 GOOS=darwin  GOARCH=arm64 go build ${LDFLAGS} -o=go-infoblox-exporter.darwin.arm64 .
